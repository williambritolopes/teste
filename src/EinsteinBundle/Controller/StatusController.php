...
use EinsteinBundle\Entity\Status;
...
class StatusController extends Controller
{
    ...
    public function indexAction()
    {
        ...
        $statuses = $em->getRepository('EinsteinBundle:Status')->findAll();
    ...
    {
        $status = new Status();
        $form = $this->createForm('EinsteinBundle\Form\StatusType', $status);
        ...
        if ($form->isSubmitted() && $form->isValid()) {
            ...
            return $this->redirectToRoute('einstein_status_show', array('id' => $status->getId()));
        }
    ...
    {
        $deleteForm = $this->createDeleteForm($status);
        $editForm = $this->createForm('EinsteinBundle\Form\StatusType', $status);
        $editForm->handleRequest($request);

        if ($editForm->isSubmitted() && $editForm->isValid()) {
            $this->getDoctrine()->getManager()->flush();

            return $this->redirectToRoute('einstein_status_edit', array('id' => $status->getId()));
        }
    ...
    public function deleteAction(Request $request, Status $status)
    {
        ...
        return $this->redirectToRoute('einstein_status_index');
    }
    ...
    private function createDeleteForm(Status $status)
    {
        return $this->createFormBuilder()
            ->setAction($this->generateUrl('einstein_status_delete', array('id' => $status->getId())))
            ->setMethod('DELETE')
            ->getForm()
        ;
    }
