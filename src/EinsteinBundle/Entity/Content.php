<?php

namespace EinsteinBundle\Entity;

use Doctrine\ORM\Mapping as ORM;

/**
 * Content
 */
class Content
{
    /**
     * @var string
     */
    private $issn;

    /**
     * @var string
     */
    private $articleId;

    /**
     * @var string
     */
    private $contrib;

    /**
     * @var string
     */
    private $autorNotes;

    /**
     * @var string
     */
    private $pubDate;

    /**
     * @var string
     */
    private $abstract;

    /**
     * @var string
     */
    private $keywords;

    /**
     * @var string
     */
    private $volume;

    /**
     * @var string
     */
    private $issue;

    /**
     * @var string
     */
    private $fpage;

    /**
     * @var string
     */
    private $lpage;

    /**
     * @var string
     */
    private $contentPt;

    /**
     * @var string
     */
    private $contentEn;

    /**
     * @var string
     */
    private $referencies;

    /**
     * @var string
     */
    private $title;

    /**
     * @var \DateTime
     */
    private $created;

    /**
     * @var \DateTime
     */
    private $updated;

    /**
     * @var \DateTime
     */
    private $deleted;

    /**
     * @var string
     */
    private $content;

    /**
     * @var string
     */
    private $file;

    /**
     * @var integer
     */
    private $id;

    /**
     * @var \EinsteinBundle\Entity\Magazine
     */
    private $magazine;


    /**
     * Set issn
     *
     * @param string $issn
     * @return Content
     */
    public function setIssn($issn)
    {
        $this->issn = $issn;

        return $this;
    }

    /**
     * Get issn
     *
     * @return string 
     */
    public function getIssn()
    {
        return $this->issn;
    }

    /**
     * Set articleId
     *
     * @param string $articleId
     * @return Content
     */
    public function setArticleId($articleId)
    {
        $this->articleId = $articleId;

        return $this;
    }

    /**
     * Get articleId
     *
     * @return string 
     */
    public function getArticleId()
    {
        return $this->articleId;
    }

    /**
     * Set contrib
     *
     * @param string $contrib
     * @return Content
     */
    public function setContrib($contrib)
    {
        $this->contrib = $contrib;

        return $this;
    }

    /**
     * Get contrib
     *
     * @return string 
     */
    public function getContrib()
    {
        return $this->contrib;
    }

    /**
     * Set autorNotes
     *
     * @param string $autorNotes
     * @return Content
     */
    public function setAutorNotes($autorNotes)
    {
        $this->autorNotes = $autorNotes;

        return $this;
    }

    /**
     * Get autorNotes
     *
     * @return string 
     */
    public function getAutorNotes()
    {
        return $this->autorNotes;
    }

    /**
     * Set pubDate
     *
     * @param string $pubDate
     * @return Content
     */
    public function setPubDate($pubDate)
    {
        $this->pubDate = $pubDate;

        return $this;
    }

    /**
     * Get pubDate
     *
     * @return string 
     */
    public function getPubDate()
    {
        return $this->pubDate;
    }

    /**
     * Set abstract
     *
     * @param string $abstract
     * @return Content
     */
    public function setAbstract($abstract)
    {
        $this->abstract = $abstract;

        return $this;
    }

    /**
     * Get abstract
     *
     * @return string 
     */
    public function getAbstract()
    {
        return $this->abstract;
    }

    /**
     * Set keywords
     *
     * @param string $keywords
     * @return Content
     */
    public function setKeywords($keywords)
    {
        $this->keywords = $keywords;

        return $this;
    }

    /**
     * Get keywords
     *
     * @return string 
     */
    public function getKeywords()
    {
        return $this->keywords;
    }

    /**
     * Set volume
     *
     * @param string $volume
     * @return Content
     */
    public function setVolume($volume)
    {
        $this->volume = $volume;

        return $this;
    }

    /**
     * Get volume
     *
     * @return string 
     */
    public function getVolume()
    {
        return $this->volume;
    }

    /**
     * Set issue
     *
     * @param string $issue
     * @return Content
     */
    public function setIssue($issue)
    {
        $this->issue = $issue;

        return $this;
    }

    /**
     * Get issue
     *
     * @return string 
     */
    public function getIssue()
    {
        return $this->issue;
    }

    /**
     * Set fpage
     *
     * @param string $fpage
     * @return Content
     */
    public function setFpage($fpage)
    {
        $this->fpage = $fpage;

        return $this;
    }

    /**
     * Get fpage
     *
     * @return string 
     */
    public function getFpage()
    {
        return $this->fpage;
    }

    /**
     * Set lpage
     *
     * @param string $lpage
     * @return Content
     */
    public function setLpage($lpage)
    {
        $this->lpage = $lpage;

        return $this;
    }

    /**
     * Get lpage
     *
     * @return string 
     */
    public function getLpage()
    {
        return $this->lpage;
    }

    /**
     * Set contentPt
     *
     * @param string $contentPt
     * @return Content
     */
    public function setContentPt($contentPt)
    {
        $this->contentPt = $contentPt;

        return $this;
    }

    /**
     * Get contentPt
     *
     * @return string 
     */
    public function getContentPt()
    {
        return $this->contentPt;
    }

    /**
     * Set contentEn
     *
     * @param string $contentEn
     * @return Content
     */
    public function setContentEn($contentEn)
    {
        $this->contentEn = $contentEn;

        return $this;
    }

    /**
     * Get contentEn
     *
     * @return string 
     */
    public function getContentEn()
    {
        return $this->contentEn;
    }

    /**
     * Set referencies
     *
     * @param string $referencies
     * @return Content
     */
    public function setReferencies($referencies)
    {
        $this->referencies = $referencies;

        return $this;
    }

    /**
     * Get referencies
     *
     * @return string 
     */
    public function getReferencies()
    {
        return $this->referencies;
    }

    /**
     * Set title
     *
     * @param string $title
     * @return Content
     */
    public function setTitle($title)
    {
        $this->title = $title;

        return $this;
    }

    /**
     * Get title
     *
     * @return string 
     */
    public function getTitle()
    {
        return $this->title;
    }

    /**
     * Set created
     *
     * @param \DateTime $created
     * @return Content
     */
    public function setCreated($created)
    {
        $this->created = $created;

        return $this;
    }

    /**
     * Get created
     *
     * @return \DateTime 
     */
    public function getCreated()
    {
        return $this->created;
    }

    /**
     * Set updated
     *
     * @param \DateTime $updated
     * @return Content
     */
    public function setUpdated($updated)
    {
        $this->updated = $updated;

        return $this;
    }

    /**
     * Get updated
     *
     * @return \DateTime 
     */
    public function getUpdated()
    {
        return $this->updated;
    }

    /**
     * Set deleted
     *
     * @param \DateTime $deleted
     * @return Content
     */
    public function setDeleted($deleted)
    {
        $this->deleted = $deleted;

        return $this;
    }

    /**
     * Get deleted
     *
     * @return \DateTime 
     */
    public function getDeleted()
    {
        return $this->deleted;
    }

    /**
     * Set content
     *
     * @param string $content
     * @return Content
     */
    public function setContent($content)
    {
        $this->content = $content;

        return $this;
    }

    /**
     * Get content
     *
     * @return string 
     */
    public function getContent()
    {
        return $this->content;
    }

    /**
     * Set file
     *
     * @param string $file
     * @return Content
     */
    public function setFile($file)
    {
        $this->file = $file;

        return $this;
    }

    /**
     * Get file
     *
     * @return string 
     */
    public function getFile()
    {
        return $this->file;
    }

    /**
     * Get id
     *
     * @return integer 
     */
    public function getId()
    {
        return $this->id;
    }

    /**
     * Set magazine
     *
     * @param \EinsteinBundle\Entity\Magazine $magazine
     * @return Content
     */
    public function setMagazine(\EinsteinBundle\Entity\Magazine $magazine = null)
    {
        $this->magazine = $magazine;

        return $this;
    }

    /**
     * Get magazine
     *
     * @return \EinsteinBundle\Entity\Magazine 
     */
    public function getMagazine()
    {
        return $this->magazine;
    }
}
